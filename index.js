...
app.put('/users/:Username', [
    check('Username', 'Username must be at least 5 characters long').optional({checkFalsy:true}).isLength({min: 5}),
    check('Username', 'Username contains non alphanumeric characters - not allowed').optional({checkFalsy:true}).isAlphanumeric(),
    check('Email', 'Email does not appear to be valid').optional({checkFalsy:true}).isEmail(), //.optional() makes the field option, {checkFalsy:True} is included because it is necessary if field is an empty string
